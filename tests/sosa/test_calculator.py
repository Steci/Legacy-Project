...
def test_sosa_of_branch_inverts_branch_lookup():
    persons, families = build_simple_tree()

    branch = branch_of_sosa(persons, families, root_id=1, number=6)
    assert branch is not None
    assert sosa_of_branch(persons, families, branch=branch) == 6

    branch = branch_of_sosa(persons, families, root_id=1, number=5)
    assert branch is not None
    assert sosa_of_branch(persons, families, branch=branch) == 5


def test_sosa_of_branch_handles_inconsistent_paths():
    persons, families = build_simple_tree()

    assert sosa_of_branch(persons, families, branch=[4, 3, 1]) is None

    with pytest.raises(ValueError):
        sosa_of_branch(persons, families, branch=[])
