...
from parsers.gw.models import Family, GWDatabase
...
def test_parse_file(parser: GWParser) -> None:
...
def test_parse_text(parser: GWParser) -> None:
    text = """
...
def test_parse_family_line(parser: GWParser) -> None:
...
def test_parse_children_block(parser: GWParser) -> None:
    lines = [
        "beg",
        "- h Corno Yann 1935 #bp Soisy 1997",
        "- f Thomas Marie_Julienne 0",
...
def test_parse_child_line(parser: GWParser) -> None:
    line = "- h Corno Yann 1935 #bp Soisy 1997"
    family = Family()
    gender, person, _ = parser._parse_child_line(line, family)
    assert gender == "h"
...
def test_maybe_add_person(parser: GWParser) -> None:
    raw = "Corno Yann 1935"
...
def test_parse_text_computes_consanguinity(parser: GWParser) -> None:
    text = """
    Smith John 1900
    Doe Jane 1905
...
def test_parse_text_cycle_records_warning(parser: GWParser) -> None:
    text = """
    Ancestor Adam 1800
    Ancestor Eve 1805
...
def test_loader_auto_refreshes_consanguinity() -> None:
    text = """
    Smith John 1900
    Doe Jane 1905
    fam Smith John + Doe Jane
...
def test_loader_cycle_collects_warning() -> None:
    text = """
    Ancestor Adam 1800
    Ancestor Eve 1805
    fam Ancestor Adam + Ancestor Eve
